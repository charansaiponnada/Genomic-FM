...
% ── Abstract ──────────────────────────────────────────────────────────
\begin{abstract}
Genomic sequence modeling requires architectures capable of processing long context windows while retaining fine-grained evolutionary patterns. State Space Models (SSMs), such as Mamba, achieve linear-time scaling $\mathcal{O}(N)$ but are fundamentally constrained by a fixed hidden state capacity ($d_{\text{state}}$), creating a bottleneck for long-range sequence compression. Conversely, Modern Hopfield Networks (MHNs) exhibit exponential storage capacity but scale quadratically with sequence length. We present \textbf{HopField-Mamba}, a novel architecture that explicitly integrates Modern Hopfield associative memory into the recurrence loop of selective State Space Models. By routing sequential representations through an energy-based memory retrieval step, HopField-Mamba enables content-addressable memory lookup with $\mathcal{O}(N)$ inference complexity. We evaluate our hybrid architecture against a pure Mamba baseline in a masked autoencoder pre-training setup across genomes of five evolutionary distinct species (Human, Mouse, Zebrafish, Drosophila, and Arabidopsis). In downstream evaluations, both architectures achieve equivalent promoter detection accuracy, while HopField-Mamba demonstrates tighter generalization variance across evolutionary distances (a 6.7 pp accuracy range compared to Mamba's 7.0 pp range). We detail the mathematical formulation of this hybrid architecture and critique its performance to guide future development.
\end{abstract}
...
* **Memory slots** $P = 1024$ (exclusive to HopField-Mamba, representing 27.1M parameters vs. Mamba's 16.1M parameters).
...
\subsection{Downstream Promoter Evaluation}
To evaluate representation quality, we train a frozen linear probe classifier on top of both encoders to perform promoter detection (identifying sequence start coordinates). The evaluation is performed on a balanced, stratified test set of 200 samples.

\begin{table}[H]
\centering
\caption{Downstream promoter detection performance.}
\label{tab:promoter}
\begin{tabular}{lcc}
\toprule
\textbf{Model} & \textbf{Accuracy (\%)} & \textbf{AUROC} \\
\midrule
Mamba MAE & 100.0\% & 1.0000 \\
HopField-Mamba & 99.5\% & 1.0000 \\
\bottomrule
\end{tabular}
\end{table}

Both architectures achieve near-perfect performance on the downstream evaluation, highlighting a clear performance ceiling on the synthetic promoter dataset.
...
While the HopField-Mamba architecture demonstrates stable pre-training and slightly tighter cross-species variance, it does not show a significant downstream performance improvement over standard Mamba in its current configuration. 

Future work should replace synthetic promoter classification with the standardized \textbf{Genomic Understanding Evaluation (GUE)} benchmark to bypass current ceiling effects. Additionally, incorporating reverse-complement bidirectional scanning (similar to Caduceus) and performing hyperparameter sweeps on the memory capacity parameter $P$ and writing rate $\lambda$ will be critical to fully leverage the associative memory capacity.
